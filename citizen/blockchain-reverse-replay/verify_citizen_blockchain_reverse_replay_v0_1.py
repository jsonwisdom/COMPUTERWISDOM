...
def main():
    ...
    assert seed["transaction"]["status"] == "HOLD"
    assert seed["receipt_and_event_log"]["status"] == "HOLD"
    assert seed["reverse_replay"]["terminal"] in TERMINALS
    assert seed["reverse_replay"]["terminal"] == "HOLD"
    assert seed["reverse_replay"]["independent_chain_replay"] == "HOLD"
    ...
    print("CITIZEN_LEDGER_ITEM_001=HOLD_INDEPENDENT_CHAIN_REPLAY")
